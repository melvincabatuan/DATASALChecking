...
public class FacePamphletCanvas extends GCanvas
    ...
    private void addQuote(String quote) {
        GLabel Quote = new GLabel(quote);
        Quote.setFont(PROFILE_STATUS_FONT);
        double x = LEFT_MARGIN + 200;
        double y = TOP_MARGIN + nameHeight + IMAGE_MARGIN + IMAGE_HEIGHT + STATUS_MARGIN + Quote.getHeight();
        if (getElementAt(x, y) != null) {
            remove(getElementAt(x, y));
        }
        add(Quote, x, y);
